...
| Logging configuration visibility | `oci_email_logging_status` inventories visible OCI service-log configuration and blocks when no active Email Delivery service log is visible; when a resource id is supplied it reports active matching resource-log counts, without enabling or mutating logs. | implemented with fixture tests; pending live configured-alias proof after hosted artifact install |
| Logging enablement plan | `oci_email_logging_enablement_plan` turns the current logging-status readback into a redacted, no-mutation operator plan for Email Domain service-log categories, approval gates, and post-enable proof. | implemented with fixture tests; pending hosted artifact install |
