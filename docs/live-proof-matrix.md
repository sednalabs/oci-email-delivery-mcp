...
| Composed watch-window proof | `oci_email_watch_window` combines profile status, resource-scoped logging-status proof, scoped metrics, scoped events, optional trace, and suppression deltas for one UTC window while still returning `send_authorized=false`. | implemented with fixture tests; pending live configured-alias proof after hosted artifact install |
