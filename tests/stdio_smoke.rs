...
fn stdio_tools_list_includes_input_schemas() {
    ...
    let engagement = tools
        .iter()
        .find(|tool| tool["name"] == "oci_email_message_engagement")
        .expect("message-engagement tool");
    assert_eq!(
        engagement["inputSchema"]["required"],
        json!(["start_time", "end_time", "message_id"])
    );
    assert!(engagement["inputSchema"]["properties"]
        .as_object()
    ...
    let logging_enablement_plan = tools
        .iter()
        .find(|tool| tool["name"] == "oci_email_logging_enablement_plan")
        .expect("logging enablement plan tool");
    assert!(logging_enablement_plan["inputSchema"]["required"].is_null());
